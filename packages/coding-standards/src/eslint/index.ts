...
import type { ExtendsElement, Config } from "@eslint/config-helpers";
...
if (
	reactPlugin.configs.flat.recommended === undefined ||
	reactPlugin.configs.flat["jsx-runtime"] === undefined
) {
	// Throw is needed to fix types correctly
	// This should never happen so if it does, it's an error worthy of breaking all eslint configs.
	throw new Error("Failed to get config from eslint-plugin-react.");
}
